...
- Since we are making the `module` option configurable by the user, we will have to introduce a relabelling rule to pass this value as a query parameter. We will no longer need the `params` block to define a static `module` option.
...
With this approach, we only need to maintain a single job definition, which will probably not require frequent changes in the long term. Users are free to define any custom labels they want, all managed by the backend that is serving the HTTP Service Discovery endpoint.
