...
summary: 'Building a scalable and maintainable system for monitoring endpoints using Prometheus and Blackbox Exporter.'
---
...
- According to the [docs](https://prometheus.io/docs/prometheus/latest/http_sd/#http_sd-format), new labels are assigned to the `labels` array, where each label key is defined using this naming convention: `__meta__<label>`.
...
- We can now introduce new relabelling rules to enrich each endpoint's metrics with the new labels.
...
To validate if the new labels are extracted properly, we can inspect the Prometheus UI's targets health page: `http://localhost:9090/targets`.
...
## Source Code
If you are interested to learn and set up the implementations covered in this blog, feel free to explore the repos below.
