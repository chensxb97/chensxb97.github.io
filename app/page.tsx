export default function Page() {
  return (
    <section>
      <h1 className="mb-6 text-2xl font-semibold tracking-tighter">
        hi there, I'm Benedict 👨‍💻
      </h1>
      <p className="mb-4">
        A software engineer, rock-climber and coffee-lover.
      </p>
      <p className="mb-4">
        I currently work as a full-stack developer in Visa's Observability team, where I design and develop new features and applications to support infrastructure monitoring and logging.
      </p>
      <p className="mb-4">
        When I am not writing code, you’ll often catch me climbing, experimenting new coffee recipes or planning my next vacation.
      </p>
    </section >
  )
}
